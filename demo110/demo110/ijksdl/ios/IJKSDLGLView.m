...
@implementation NTESIJKSDLGLView {
...
- (void)layoutSubviews
{
    ...
    newFrame.size.width   = selfFrame.size.width * 1 / 2;
    newFrame.origin.x     = selfFrame.size.width * 1 / 2;
...
- (void)display: (SDL_VoutOverlay *) overlay clear:(BOOL)clear
{
    ...
    if (_context && !_didStopGL) {
        EAGLContext *prevContext = [EAGLContext currentContext];
        [EAGLContext setCurrentContext:_context];
        [self displayInternal:overlay clear:clear];
        [EAGLContext setCurrentContext:prevContext];
    }
...
- (void)displayInternal: (SDL_VoutOverlay *) overlay clear:(BOOL)clear
{
